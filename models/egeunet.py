...
class EGEUNet(nn.Module):
    ...
    def __init__(self, num_classes=1, input_channels=3, c_list=[8,16,24,32,48,64], bridge=True, gt_ds=True):
        ...
        if gt_ds:
            self.gt_conv1 = nn.Sequential(nn.Conv2d(c_list[4], num_classes, 1))
            self.gt_conv2 = nn.Sequential(nn.Conv2d(c_list[3], num_classes, 1))
            self.gt_conv3 = nn.Sequential(nn.Conv2d(c_list[2], num_classes, 1))
            self.gt_conv4 = nn.Sequential(nn.Conv2d(c_list[1], num_classes, 1))
            self.gt_conv5 = nn.Sequential(nn.Conv2d(c_list[0], num_classes, 1))
            self.gt_conv1_out = nn.Sequential(nn.Conv2d(num_classes, 1, 1))
            self.gt_conv2_out = nn.Sequential(nn.Conv2d(num_classes, 1, 1))
            self.gt_conv3_out = nn.Sequential(nn.Conv2d(num_classes, 1, 1))
            self.gt_conv4_out = nn.Sequential(nn.Conv2d(num_classes, 1, 1))
            self.gt_conv5_out = nn.Sequential(nn.Conv2d(num_classes, 1, 1))
    ...
    def forward(self, x):
        ...
        if self.gt_ds: 
            ...
            gt_pre5_out = self.gt_conv1_out(gt_pre5)
            t5 = self.GAB5(t6, t5, gt_pre5_out)
        ...
        if self.gt_ds: 
            ...
            gt_pre4_out = self.gt_conv2_out(gt_pre4)
            t4 = self.GAB4(t5, t4, gt_pre4_out)
        ...
        if self.gt_ds: 
            ...
            gt_pre3_out = self.gt_conv3_out(gt_pre3)
            t3 = self.GAB3(t4, t3, gt_pre3_out)
        ...
        if self.gt_ds: 
            ...
            gt_pre2_out = self.gt_conv4_out(gt_pre2)
            t2 = self.GAB2(t3, t2, gt_pre2_out)
        ...
        if self.gt_ds: 
            ...
            gt_pre1_out = self.gt_conv5_out(gt_pre1)
            t1 = self.GAB1(t2, t1, gt_pre1_out)
        ...
        if self.gt_ds:
            return (
                torch.softmax(gt_pre5, dim=1),
                torch.softmax(gt_pre4, dim=1),
                torch.softmax(gt_pre3, dim=1),
                torch.softmax(gt_pre2, dim=1),
                torch.softmax(gt_pre1, dim=1)
            ), torch.softmax(out0, dim=1)
        else:
            return torch.softmax(out0, dim=1)