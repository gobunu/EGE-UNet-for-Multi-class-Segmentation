...
class GT_BceDiceLoss(nn.Module):
    ...
    def forward(self, gt_pre, out, target):
        ...
        # print(gt_pre5.shape, gt_pre4.shape, gt_pre3.shape, gt_pre2.shape, gt_pre1.shape)
        # time.sleep(100)
...
class myToTensor:
    ...
    def __call__(self, data):
        image, mask = data
        if mask is None:
            return torch.tensor(image).permute(2,0,1), None
...
class myResize:
    ...
    def __call__(self, data):
        image, mask = data
        if mask is None:
            return TF.resize(image, [self.size_h, self.size_w]), None
